    ...
    public static partial class Expr
    {
            ...
            ExpressionVisitor
        {
            protected override Expression VisitMethodCall(MethodCallExpression node)
            {
                ...
                if (node.TargetsMethod(typeof(Expr), nameof(Expr.Capture)))
                {
                ...
                if (node.TargetsMethod(typeof(Expr), nameof(Expr.Invoke)))
                {
                    var target = node.Arguments.First();
            ...
            /// <summary>
            /// Evaluates the <paramref name="target"/> expression passed to <see cref="Expr.Invoke{TResult}"/> calls
